...
from data.config import IG_USERNAME, IG_PASSWORD, WEBAPP_HOST
...
async def check_stories():
    ...
    for thread in pending_threads:
        ...
        if item_type == "reel_share" and item["reel_share"]["mentioned_user_id"] == inst.user_id:
            ...
            user_image_stories = await get_user_image_stories(ig_user_id, inst)
            count_stories = len(user_image_stories)
            banana_num = await show_last_stories(user_image_stories, count=3) if count_stories > 0 else None
            if banana_num:
                await notify_succeeded_stories(ig_username, banana_num)
            elif count_stories > 0 or len(await get_user_video_stories(ig_user_id, inst)) > 0:
                await notify_failed_stories(ig_username)
...
@sync_to_async
def get_user_image_stories(ig_user_id: str, inst_bot: instabot.Bot):
    list_image_stories = inst_bot.get_user_stories(ig_user_id)[0]
    return list_image_stories


@sync_to_async
def get_user_video_stories(ig_user_id: str, inst_bot: instabot.Bot):
    list_video_stories = inst_bot.get_user_stories(ig_user_id)[1]
    return list_video_stories
...
async def notify_succeeded_stories(ig_username: str, banana_num: int):
    async with aiohttp.ClientSession() as session:
        url = WEBAPP_HOST + "/notify_succeeded_stories"
...
async def notify_failed_stories(ig_username: str):
    async with aiohttp.ClientSession() as session:
        url = WEBAPP_HOST + "/notify_failed_stories"
